	...
	constructor(props) {
	  ...
	  this.trans = function(){
	  	if (__this.state.transUS) {
		   ...
		   __this.setState({CHMoney:(us*29.52).toFixed(2).toString()});
		}else{
			let ch = parseFloat(__this.state.CHMoney);
		   __this.setState({USMoney:(ch/29.52).toFixed(2).toString()});
		}
	  }
	}

	render(){
		let transText = this.state.transUS?"美金":"新台幣";
		let transToText = this.state.transUS?"新台幣":"美金";
		let topText = this.state.transUS?this.state.USMoney+'$':this.state.CHMoney+'NTD';
		let bottomText = this.state.transUS?this.state.CHMoney+'  NTD':this.state.USMoney+'$';
		return(
